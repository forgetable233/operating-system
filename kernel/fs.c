...
static void mkfs()
{
	...
	WR_SECT_BUF(orange_dev, 2 + sb.nr_imap_sects + sb.nr_smap_sects, fsbuf);	//modified by mingxuan 2020-10-27
...
static int search_file(char * path)
{
	...
	for (i = 0; i < nr_dir_blks; i++) {
		...
		RD_SECT_BUF(dir_inode->i_dev, dir_blk0_nr + i, fsbuf);	//modified by mingxuan 2019-5-20
	...
	if (imode == I_CHAR_SPECIAL) {
		...
		for (i = rw_sect_min; i <= rw_sect_max; i += chunk) {
			...
			if (fs_msg->type == READ) {
				...
				// 	  fsbuf);
				// kprintf("write sector\n");
				WR_SECT(pin->i_dev, i, fsbuf);
			}
			off = 0;
			bytes_rw += bytes;
